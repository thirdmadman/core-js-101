...
function logger(func, logFunc) {
  return (...args) => {
    const argsToString = (...inputArgs) => inputArgs.map((el) => JSON.stringify(el)).join(',');
    logFunc(`${func.name}(${argsToString(...args)}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argsToString(...args)}) ends`);
    return result;
  };
}
